...
def _generate_random_name(length: int, avoid_names: set[str]) -> str:
    chars = string.ascii_letters
    for _ in range(100):
        new_name = "".join(random.choice(chars) for _ in range(length))
        if random.random() < 0.2:
            new_name += random.choice(SUBSCRIPTS)
...
def _random_rename_variables_in_goal(goal: LeanGoal) -> LeanGoal:
    ...
    for i in range(len(current_hypotheses)):
        ...
        if random.random() < 0.5:
            new_name = _generate_random_name(len(old_name), avoid_names)
    ...
    return goal.with_(hypotheses=current_hypotheses, type=current_goal_type)

def random_rename_variables(node: ProofTreeNode) -> ProofTreeNode:
    new_goals = [_random_rename_variables_in_goal(g) for g in node.state.goals]
    return node.with_(state=LeanProofState(new_goals))

def random_rename_goals(node: ProofTreeNode) -> ProofTreeNode:
    avoid_names = set()
    ...
    for g in node.state.goals:
        old_name = g.tag
        length = len(old_name) if old_name else random.randint(1, 5)
        new_name = _generate_random_name(length, avoid_names)
        avoid_names.add(new_name)
        ...
        for theorem in lean_file.theorems:
            ...
            for block in theorem.by_blocks:
                ...
                for i, node in enumerate(nodes):
                    if i > 5: break  # Limit to first 5 nodes per tree to avoid spam
                    ...
                    if node.state:
                        ...
                        new_node = random_rename_variables(node)
